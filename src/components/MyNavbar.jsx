        ...
        <Navbar className='w-100'>
            <Container className='container-fluid'>
                <div className="logoContainer d-flex align-items-center">
                    <Navbar.Brand href='/'><img src={logo} className='logo' alt='Pizza logo' /></Navbar.Brand>
                    <h4 className='m-0'>Mamma Mia!</h4>
                </div>
                <Navbar.Toggle />
                <Navbar.Collapse className="justify-content-end position-relative">
                    {cart.length > 0 ? <span className='cartCount position-absolute d-flex align-items-center justify-content-center'>{cart.length}</span> : ''}
                    {cart.length > 0 ? <span className='cartCount position-absolute d-flex align-items-center justify-content-center'>{cart.length}</span> : ''}
